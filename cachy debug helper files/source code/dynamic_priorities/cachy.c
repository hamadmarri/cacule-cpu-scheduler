...
int hrrn_max_lifetime					= 600000;	// in ms
...
pick_next_entity(struct cfs_rq *cfs_rq, struct sched_entity *curr)
{
	...
	if (curr && wakeup_preempt_entity(now, se, curr) == 1)
		se = curr;
...
wakeup_preempt_entity(u64 now, struct sched_entity *curr, struct sched_entity *se)
{
	u64 r_curr, r_se, l_curr = 1ULL, l_se = 1ULL;
	...
	if (vr_curr == 1) {
		...
		//printk("************* %d %llu", task_of(curr)->pid, vr_curr);
	}
	
	if (vr_se == 1) {
		se->hrrn_start_time = now - 1000000ULL;
		//printk("************* %d %llu", task_of(se)->pid, vr_se);
	}
...
simple:
	...
	if (p->prio > (int)(p->se.hrrn_sum_exec_runtime)) {
		int new_prio = p->static_prio - 1;
...
static void task_fork_fair(struct task_struct *p)
{
	...
	p->se.hrrn_start_time = p->start_time + (hrrn_latency * 1000ULL);
	...
	if (curr) {
		...
		prio += curr->hrrn_sum_exec_runtime;
		...
		//printk("*********** %s c->hrrn %llu, c->sp %d, p->sp %d, p %d, np %d",
			//p->comm, curr->hrrn_sum_exec_runtime,
			//task_of(curr)->static_prio, p->static_prio, prio, new_prio);

		p->se.hrrn_sum_exec_runtime = prio;
		p->static_prio = new_prio;
		p->prio = p->normal_prio = new_prio;
		reweight_task(p, new_prio - MAX_RT_PRIO);
